    ...
    _originals(NULL)
{
    ...
    _selectCount = g_slist_length((GSList *)selectedReprList);

    // Init the data-holders
    _nodes = new Inkscape::XML::Node*[_selectCount];
    _originals = new const char*[_selectCount];
    _caches = new char*[_selectCount];
    _cacheLengths = new unsigned int[_selectCount];
    _images = new Magick::Image*[_selectCount];
    _imageCount = 0;
    ...
    for (; selectedReprList != NULL; selectedReprList = g_slist_next(selectedReprList)) {
        ...
        if (!strcmp(node->name(), "image") || !strcmp(node->name(), "svg:image")) {
            ...
            // Set to the empty string 0x0, which must be stored in heap
            _caches[_imageCount] = new char(0);
...
void
ImageMagick::effect(Inkscape::Extension::Effect *module,
...
*/
Gtk::Widget *
ImageMagick::prefs_effect(Inkscape::Extension::Effect *module,
